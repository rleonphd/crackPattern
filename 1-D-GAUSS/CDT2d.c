...
**/
#define EPS 1.0E-16
void GaussIntersectionPolygon(CDT2d *cdt,unsigned long i,unsigned long ip,LINE dirLine,double sigma,gsl_rng *r, unsigned long lOption){
	unsigned long j, k, k1, k2;
	// k0[1] - k0[0] is the number of vertices between the first and the second intersection point.
	unsigned long k0[2]; 
	// ind means index of intersection points (0 and 1).
...
**/
#define EPS 1.0E-16
void GaussIntersectionPolygonDisturbed(CDT2d *cdt,unsigned long i,unsigned long ip,LINE dirLine,double sigma,gsl_rng *r, double bEllip, unsigned long lOption){
	unsigned long j, k, k1, k2;
	// k0[1] - k0[0] is the number of vertices between the first and the second intersection point.
	unsigned long k0[2]; 
	// ind means index of intersection points (0 and 1).
...
**/  
int STIT2dGauss(CDT2d *cdt, double timeGaussStop, double sigma, unsigned long lOption){
	unsigned long i;
	...
	do{
		...
		for(i = 0; i < numPol; i++){
			...
			if(cdt->GaussPolygon[i].time < timeGaussStop){
				...
				GaussIntersectionPolygon(cdt, i, numPol + q, dirLine, sigma, r, lOption);
				q++;
				(cdt->numberOfGaussPolygons)++;
				(cdt->numberOfI_Segments)++;
			}
...
/*
*/
int STIT2dGaussDisturbed(CDT2d *cdt, double timeGaussStop, double sigma, double bEllip, unsigned long lOption){
	unsigned long i;
	...
	do{
		...
		for(i = 0; i < numPol; i++){
			...
			if(cdt->GaussPolygon[i].time < timeGaussStop){
				...
				GaussIntersectionPolygonDisturbed(cdt, i, numPol + q, dirLine, sigma, r, bEllip, lOption);
				q++;
				(cdt->numberOfGaussPolygons)++;
				(cdt->numberOfI_Segments)++;
			}
