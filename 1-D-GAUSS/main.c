...
STIT Anisotropic / Anisotropic Disturbed:
	...
	argv[8]: angle dir[0]
	argv[9]: probability dir[0]
	...
	argv[8 + 2*nDir - 2]: angle dir[nDir - 1]
	argv[8 + 2*nDir - 1]: probability dir[nDir - 1]
...
int main(int argc, char *argv[]){
	...
	if (option == 0){
		...
		//Gauss modification
		NoBoundary(&cdt);
		STIT2dGauss(&cdt, timeGaussStop, sigma, lifeOption);
	}	
	// STIT Anisotropic
		...
		if (option == 1){
			...
			//Gauss modification
			NoBoundary(&cdt);
			STIT2dGaussDisturbed(&cdt, timeGaussStop, sigma, bEllip, lifeOption);
		}
